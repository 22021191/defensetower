...
public class Hero : MonoBehaviour
{
    ...
    // Hàm tự động hồi máu khi không có mục tiêu tấn công
    private void Regend() 
    {
    ...
    // Kiểm tra xem Hero có phải đánh nhau đến chết với Enemy này không
    ...
    private void AttackEnemy() // Hàm tấn công Enemy
    {
        if (isAttacking == false)
        {
            ...
            if (_random == 4 || _random == 5 || _random == 6) // có 30% tỷ lệ chạy Animation chém bình thường
            {
                if (!isRange)
                    _anim.SetInteger("Change", 2);
                else
